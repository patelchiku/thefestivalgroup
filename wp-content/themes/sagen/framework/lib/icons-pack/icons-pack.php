...
class SagenSelectClassIconCollections {
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		$iconCollectionsVCMap  = get_transient( 'sagen_select_social_icon_collections' ) ?? array();

		if ( ! empty( $iconCollectionsVC ) && empty( $iconCollectionsVCMap ) ) {
			foreach ( $iconCollectionsVC as $collection_key => $collection ) {
				$iconCollectionsVCMap[ $collection_key ] = array(
					'param_name' => $collection->param,
					'icon_name'  => $collection->title,
					'icons'      => $collection->getSocialIconsArrayVC(),
				);
			}

			set_transient( 'sagen_select_social_icon_collections', $iconCollectionsVCMap, WEEK_IN_SECONDS );
		}
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVCMap ) && count( $iconCollectionsVCMap ) ) {
				foreach ( $iconCollectionsVCMap as $collection_key => $collection ) {
					$iconOps[] = array(
						'type'    => 'dropdown',
						'name'    => $collection['param_name'] . '_' . $n,
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'sagen' ), $n, $collection['icon_name'] ),
						'options' => array_flip( $collection['icons'] )
					);
				}
			}
