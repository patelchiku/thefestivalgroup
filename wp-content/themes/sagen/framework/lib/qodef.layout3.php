...
class SagenSelectClassMultipleImages implements iSagenSelectInterfaceRender {
	...
	function __construct( $name, $label = '', $description = '' ) {
		...
		$sagen_select_global_Framework->qodeMetaBoxes->addOption( $this->name, '', 'image' );
	}
			...
			<div class="qodef-section-content">
				<div class="container-fluid">
					...
					if ( $instagram_user_id ) {
						...
						<div class="row">
							<div class="col-lg-12">
								<p>
								<?php
								echo esc_html__( 'You are currently connected to Instagram ID: ', 'sagen' );
									echo esc_attr( $instagram_user_id )
								?>
									</p>
							</div>
						</div>
					...
					<div class="row">
						...
						<?php } elseif ( empty( $connection_type ) ) { ?>
							<div class="col-lg-4">
...
class SagenSelectClassRepeater implements iSagenSelectInterfaceRender {
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false ) {
		...
		$sagen_select_global_Framework->qodeMetaBoxes->addOption( $this->name, '', 'repeater' );
	}
	...
	public function render( $factory ) {
		...
		<div class="qodef-repeater-wrapper <?php echo implode( ' ', $wrapper_classes ); ?>">
			...
			<div class="qodef-repeater-wrapper-inner <?php echo esc_attr( $sortable_class ); ?>" data-template="<?php echo str_replace( '_', '-', $this->name ); ?>">
				<?php
				if ( ! empty( $clones ) && is_array($clones) && count($clones) > 0 ) {
					$counter = 0;
									...
									if ( $field['type'] == 'repeater' ) {
										...
										?>
												...
												<?php if ( $field['description'] != '' ) { ?>
														...
														<?php } ?>
												<?php
												if ( ! empty( $clone[ $field['name'] ] ) && is_array($clone[ $field['name'] ]) && count($clone[ $field['name'] ]) > 0 ) {
													$counter2 = 0;
																...
																foreach ( $field['fields'] as $field_inner ) {
																	...
																	?>
																			...
																			if ( ! empty( $field_inner['dependency'] ) ) {
																				$dependencyValues = sagen_select_return_repeater_dependency_options_array(
																					array(
																						'field'        => $field,
						...
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							<select name="<?php echo esc_attr( $name ) . '[icon_pack]'; ?>" class="<?php echo esc_attr( $select2 ); ?> form-control qodef-form-element icon-dependence">
								...
								foreach ( $icon_packs as $key => $value ) {
									...
									<option 
									<?php
									if ( ! empty( $rvalue ) && $rvalue['icon_pack'] == $key ) {
