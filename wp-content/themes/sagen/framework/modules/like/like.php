...
class SagenSelectClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_sagen_select_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_sagen_select_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'sagen' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'sagen' );
		}
...
if ( ! function_exists( 'sagen_select_create_like' ) ) {
	function sagen_select_create_like() {
		SagenSelectClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'sagen_select_create_like' );
}