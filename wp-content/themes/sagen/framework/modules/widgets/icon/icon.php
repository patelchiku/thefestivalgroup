...
if ( class_exists( 'SagenCoreClassWidget' ) ) {
	class SagenSelectClassIconWidget extends SagenCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'qodef_icon_widget',
				esc_html__( 'Sagen Icon Widget', 'sagen' ),
				array( 'description' => esc_html__( 'Add icons to widget areas', 'sagen' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				sagen_select_icon_collections()->getIconWidgetParamsArray(),
				array(
					array(
						'type'  => 'textfield',
						'name'  => 'icon_text',
						'title' => esc_html__( 'Icon Text', 'sagen' )
					),
					array(
						'type'  => 'textfield',
						'name'  => 'link',
					...
					array(
						'type'  => 'textfield',
						'name'  => 'text_size',
						'title' => esc_html__( 'Text Size (px)', 'sagen' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'text_color',
						'title' => esc_html__( 'Text Color', 'sagen' )
					),
					array(
						'type'        => 'textfield',
						'name'        => 'margin',
		...
		public function widget( $args, $instance ) {
			...
			$data   = array();
			$data[] = ! empty( $instance['icon_hover_color'] ) ? sagen_select_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
			$data   = implode( ' ', $data );
			...
			if ( ! empty( $instance['text_size'] ) ) {
				$text_styles[] = 'font-size: ' . sagen_select_filter_px( $instance['text_size'] ) . 'px';
			}
			...
			if ( ! empty( $instance['icon_pack'] ) ) {
				...
				if ( ! empty( $icon_class ) ) {
					...
					$icon_holder_html = '<span class="qodef-icon-element ' . esc_attr( $icon_class ) . '" ' . sagen_select_get_inline_style( $icon_styles ) . '></span>';
				}
			}
			...
			if ( ! empty( $instance['icon_text'] ) ) {
				$icon_text_html = '<span class="qodef-icon-text ' . esc_attr( $icon_text_class ) . '" ' . sagen_select_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
			}
			?>
			
			<a <?php sagen_select_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo sagen_select_get_inline_style( $holder_styles ); ?>>
