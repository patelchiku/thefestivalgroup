...
$qodef_row_wrapper_start = $qodef_row_wrapper_end = $row_background_corner = $row_background_text = $row_background_text_1 = $row_background_text_size = $row_background_text_color = $row_background_text_size_1440 = $row_background_text_size_1280 = '';
...
if(!empty($row_background_text_1)) {
	...
	$row_background_text_1_style = array();
	...
	if(!empty($row_background_text_padding_right)){
		$row_background_text_style[] = 'padding-right:' . esc_html($row_background_text_padding_right);
	}

	if(!empty($row_background_text_padding_bottom)){
		$row_background_text_1_style[] = 'padding-bottom:' . esc_html($row_background_text_padding_bottom);
	}

	$row_background_text_styles = implode(';', $row_background_text_style);
	$row_background_text_1_style = implode(';', $row_background_text_1_style);
	...
	$row_background_text = '<div class="qodef-row-background-custom-holder">';
	$row_background_text .= '<div class="qodef-row-background-text-wrapper '.esc_attr($row_background_text_wrapper_classes).'">';
	$row_background_text .= '<div class="qodef-row-background-text-wrapper-inner" ' . sagen_select_get_inline_style($row_background_text_styles) . ' ' . esc_html($row_background_text_datas) . '>';

	$row_background_text .= '<div class="qodef-row-background-text-1" '. sagen_select_get_inline_style($row_background_text_1_style).'>';
...
if ( $row_content_width === 'grid' ) {
	$qodef_row_wrapper_start .= '<div class="qodef-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . sagen_select_get_inline_style( $qodef_grid_row_styles ) . '><div class="qodef-row-grid-section">';
	$qodef_row_wrapper_end   .= '</div></div>';
}

$qodef_after_wrapper_open   = apply_filters('sagen_select_filter_vc_after_wrapper_open', '', $atts);
$qodef_before_wrapper_close = apply_filters('sagen_select_filter_vc_before_wrapper_close', '', $atts);
$css_classes                = apply_filters('sagen_select_filter_vc_css_classes', $css_classes, $atts);
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . sagen_select_get_inline_style( $qodef_vc_row_styles ) . '>';
$output .= $qodef_after_wrapper_open;
$output .= wpb_js_remove_wpautop( $content );
if($row_background_text !== '') {
	$output .= $row_background_text;
}
if($row_background_corner !== '') {
	$output .= '<div class="qodef-row-background-corner-holder ' . $row_background_corner . '"></div>';
}
...
echo sagen_select_get_module_part( $output );
