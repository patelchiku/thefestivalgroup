<?php
namespace SagenCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'sagen-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'sagen-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'sagen-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'sagen-core' )   => 'qodef-visible-on-hover',
								esc_html__( 'Visible on Default', 'sagen-core' ) => 'qodef-visible-on-default',
								esc_html__( 'Disabled', 'sagen-core' )           => 'qodef-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'sagen-core' ),
							'value'       => array(
								esc_html__( 'Default', 'sagen-core' )  => 'default',
								esc_html__( 'Centered', 'sagen-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'sagen-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'sagen-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
						...
						array(
							...
							'heading'     => esc_html__( 'Subtitle Tag', 'sagen-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Subtitle Color', 'sagen-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true )
						),
						array(
							'type'        => 'dropdown',
						...
						array(
							...
							'heading'     => esc_html__( 'Words with Light Font Weight', 'sagen-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "light" font weight. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have a light font weight, you would enter "1,3,4")', 'sagen-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'sagen-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'sagen-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'sagen-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = sagen_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getHolderClasses( $params, $args ) {
		...
		$holderClasses[] = ($params['enable_separator'] == 'no') ? 'qodef-banner-disable-separator' : '';
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . sagen_select_filter_px( $params['link_top_margin'] ) . 'px';
		}
